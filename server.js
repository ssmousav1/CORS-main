...
const portResponse = require("./helpers/portResponse");
const { notifications } = require("./helpers/notificationWS");
  ...
  jwt.verify(
    ...
    (err, user) => {
      if (err) {
        ...
        socket.send(JSON.stringify({ data: GPSdata }));
        socket.send(JSON.stringify({ notification: notifications }));
