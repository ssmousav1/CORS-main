...
const { messagesToWatchdog, statusMessagesToWatchdog } = require('./helpers/watchdogInterface')
// middlewares
...
setInterval(() => {
  ...
  if (GPSdata.inViewSatellites.all && GPSdata.inViewSatellites.all.length === 0) {
    if (GPSdata.configs.NMEA > 2) {
...
server.on('connection', (socket, req) => {
  ...
  jwt.verify(req.url.slice(1), process.env.ACCESS_TOKEN_SECRET || 'accessTokenSecret', (err, user) => {
    if (err) {
