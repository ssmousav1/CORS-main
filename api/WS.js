...
const deviceStatusParser = deviceStatusPort.pipe(
...
const checkDeviceStatus = (deviceStatus) => {
	return {
		'12V0_MAIN':
			parseFloat(deviceStatus['12V0_MAIN']) > 11.5 &&
			parseFloat(deviceStatus['12V0_MAIN']) < 12.5,
		'5V0_MAIN':
			parseFloat(deviceStatus['5V0_MAIN']) > 4.8 &&
			parseFloat(deviceStatus['5V0_MAIN']) < 5.2,
		'3V7_MAIN':
			parseFloat(deviceStatus['3V7_MAIN']) > 3.5 &&
			parseFloat(deviceStatus['3V7_MAIN']) < 3.9,
		'3V3_CORE':
			parseFloat(deviceStatus['3V3_CORE']) > 3 &&
			parseFloat(deviceStatus['3V3_CORE']) < 3.6,
		'3V3_GPS':
			parseFloat(deviceStatus['3V3_CORE']) > 3 &&
			parseFloat(deviceStatus['3V3_CORE']) < 3.6,
		'4V2_BATTERY':
			parseFloat(deviceStatus['3V3_CORE']) > 3.5 &&
			parseFloat(deviceStatus['3V3_CORE']) < 4.25,
		HUMIDITY:
			parseFloat(deviceStatus.HUMIDITY) > 0 &&
			parseFloat(deviceStatus.HUMIDITY) < 40,
		TEPERATURE:
			parseFloat(deviceStatus.TEPERATURE) > 10 &&
			parseFloat(deviceStatus.TEPERATURE) < 60,
		RESERVE_1: false,
		RESERVE_2: false,
	};
};
...
deviceStatusParser.on('data', (line) => {
	try {
		GPSdata.deviceStatus = {
			...GPSdata.deviceStatus,
			...JSON.parse(line),
			deviceHealth: checkDeviceStatus(JSON.parse(line)),
		};
	} catch (error) {
		console.error('Got bad packet:', line, error);
	}
